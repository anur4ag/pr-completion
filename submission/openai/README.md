# OpenAI directory submission materials (0.4.0 draft)
...
2. A full plugin ZIP without `interface.composerIcon` and `interface.logo` was rejected; both must reference a square image inside the package. The current package also includes the documented optional `interface.logoDark` asset.
...
| Skills | Generated `pr-completion-0.4.0-portal-plugin.zip` |
